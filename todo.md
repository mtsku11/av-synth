...
- [x] **Add bounded typographic / time-domain operator pack.** ✅ 2026-05-31 — Added `glyphRender`, `glyphMotion`, and `matrixRain`; extended `slitScan` with radial/spiral modes plus shared-history delay/smear/strobe controls; added low-rate bass/mid/high analyser bands for shader tint and motion; authored six programs now curated into the public nine-program bank. The pack stays inside the existing WebGL2 pass/resource model: no atlas, extra FBO system, physics engine, optical-flow expansion, or public-audio expansion.
...
- [ ] **`flow` still has a single-scale motion-estimator ceiling.** `motion-analysis.frag` now uses quality-tier resolution and confidence gating around its single-scale 5×5 Lucas-Kanade approximation. That removes avoidable low-structure jitter but large movement can still saturate. The same ceiling hits `glyphMotion` orientation/displacement and any routed `modulate*` consumer. A real multi-scale upgrade remains architectural.
