...
- [x] **Add bounded typographic / time-domain operator pack.** ✅ 2026-05-31 — Added `glyphRender`, `glyphMotion`, and `matrixRain`; extended `slitScan` with radial/spiral modes plus shared-history delay/smear/strobe controls; added low-rate bass/mid/high analyser bands for shader tint and motion; authored six Advanced programs. The pack stays inside the existing WebGL2 pass/resource model: no atlas, extra FBO system, physics engine, optical-flow expansion, or public-audio expansion.
...
- [ ] **`flow` is bottlenecked by the motion estimator.** `motion-analysis.frag` uses a half-resolution, single-scale 5×5 Lucas-Kanade approximation. It remains intentionally cheap and can saturate on large or low-texture movement. The same ceiling hits `glyphMotion` orientation/displacement and any routed `modulate*` consumer. Real fix is multi-scale motion analysis; architectural.
