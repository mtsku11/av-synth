...
- [x] **Polish the built-in footage demo and bounded motion/history quality.** ✅ 2026-05-31 — Kept the committed 10-second stereo footage clip as the paused first-run canvas source, fixed the welcome overlay so it stays until the explicit `Start Demo` gesture, added quality-tier motion-field resolution (`0.375×` Safe Mode, `0.625×` standard, `0.75×` cinema), confidence-gated low-structure flow cells, and fractional/non-linear `slitScan` history sampling. The live browser pass also normalized seven stale preset render-style tokens exposed by the new public glyph bank and fixed repeated recordings by cloning the shared audio-destination track per capture session. The temporal ring remains eight frames; no heavier optical-flow architecture or extra history allocation was added.
