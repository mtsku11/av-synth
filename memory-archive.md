# memory-archive.md — full historical decision log

Verbatim archive of `memory.md` as of 2026-06-10, plus entries pruned from the live file since.
**Do not read this file whole** — it is tens of thousands of tokens. Search it:
`grep -n '^##' memory-archive.md` for the entry index, then read the specific line range.
The live decision log is `memory.md`.
...
## 2026-05-27 — Validator tightening + discrete param-choice support + probe fixture pivot + CI gates

Three small follow-ups landed together:

- **`OperatorDef.audit` gained optional `caseOperator?: string`.** The registry test now parses each referenced `qa/cases/<id>.json` and asserts its `audit.operator` field equals `def.audit.caseOperator ?? def.op`. Channel ops set `caseOperator: 'channel'` so the shared `audit-channel-isolate.json` (whose declared operator is the family name, not the per-op id) still validates. The previous copy-paste class — case file exists but exercises a different op — now fails at registration time.
- **`ParamSpec` gained optional `choices?: readonly { value: number; label: string }[]`.** `Slider.svelte` switches to a segmented picker when present (engine still receives a number; the picker just snaps to the named values). `formatValue` returns the matching label. `slitScan.orientation` declares `[{0,'vertical'},{1,'horizontal'}]` — the per-op `Math.round` snap in `setUniforms` stays as the engine-side floor. The same shape covers any future enum/mode param without growing a parallel ParamSpec variant.
- **Probe fixture pivot — slit-scan and dataMosh now use `frame-ramp.mp4` (90 frames of monotonically-increasing solid gray, 30 fps, 3 s).** That determinism lets the gates read effect signatures directly instead of approximating them through whole-frame luma-diff on `ci-smoke`. Slit-scan now asserts column-vs-row spread asymmetry (vertical mode `colSpread / rowSpread ≥ 2`, horizontal mode swaps) and a signed-gradient flip across `slitX=0` vs `slitX=1` (gradient sign must reverse). DataMosh now asserts `identity-Δt > 8` (passthrough tracks the changing source), `identity↔hold > 8` (held image visibly differs from live), and `hold-Δt × 3 < identity-Δt` (the actual datamosh-stability signature). The dataMosh probe was also setting `hold`/`release` params that don't exist on the shipped op (they were specced but never implemented); the live/hold split now uses real params (`mix`, `drift`, `decay`, `chunk`) and the bridge warnings are gone. The earlier soft `> 0.05` luma-diff gates on `ci-smoke` are retired.
- **Three CI gates added.** `op-characterisation.yml`, `grain-composite.yml`, `granulator-density.yml` — each path-filtered to just its hot files plus the spec and playwright config. All three follow the `granulator-soak.yml` template (Node 22, npm ci, build, install chromium, run probe, upload artifact 14d). The grain-composite workflow includes a small `find … -name 'grain-composite-*.png' -exec cp` step before upload so the `/tmp` diagnostic screenshots survive into the artifact.

`datamoshHold` preset added (`feedback → dataMosh → chromaShift → contrast`, three macros). Existing `datamoshSmear` (flow-based) is kept as the softer motion-aware variant; this is the I-frame-hold flagship. Public allowlist now 17 entries.

## 2026-05-28 — Source B exposed as a graph node, not per-op routing (LANDED)

Marc loaded a second video source and could not find a way to use it in any operator other than the bespoke `sourceBlend`. The existing wiring is: a global "load source B" file picker exists, the renderer pre-renders Source B to its own FBO every frame, and that FBO is bound to texture unit 8 — but the only operator that samples unit 8 is `sourceBlend`. The 4-bus graph and the two-input operator system (`blend`, `mask`, all `modulate*Routed`) have no knowledge of Source B.

**The design choice considered and rejected**: add a per-operator "source A / source B" toggle to two-input ops. Rejected because (a) it duplicates routing surface the graph already provides via primary/secondary input pickers, (b) it would require parallel work on every two-input op rather than a single graph change, and (c) it doesn't generalise — adding a third source later would require another toggle on every op.

**The design choice taken (subject to approval)**: model Source B as a second source *node* in the graph, sitting next to `SOURCE_NODE_ID`. Every existing primary/secondary input picker that already lists `source` and `src(o0..o3)` picks it up automatically once `sourceB` is added to the option set. The four-bus layout gets a gentle convenience default — bus 1's chain-start defaults to `sourceB` when one is loaded, so monitor o0 shows Source A and monitor o1 shows Source B without further configuration. The bus default is a starting position only; the user can route either source into any bus.

**Why this is the right shape**: it reuses the routing/coupling architecture that already exists in `src/core/patch-graph.ts:198-215, 322-340` rather than building a parallel system. The dedicated `sourceBlend` operator becomes redundant with `blend(primary=source, secondary=sourceB)` and can be deprecated later. The single non-trivial implementation step is renderer-side: today `renderer.ts:1488-1495` only routes the Source B FBO to one hardcoded texture unit for one specific shader; that needs to generalise so any node whose resolved input is `SOURCE_B_NODE_ID` gets its `u_tex` bound to the Source B FBO.

**How to apply**: any future "I want to use input X in operator Y" feature should follow the same pattern — make X addressable in the graph, let existing input pickers surface it, don't add per-op routing toggles. Webcam input, MIDI-selected clip slots, and procedural pattern sources should all enter the graph as named source nodes.

**Open question recorded for Marc**: stop at exactly two source slots for v1, or design upfront for N sources? Current recommendation is two sources for v1 and generalise when a third concrete caller appears (rule of three).
