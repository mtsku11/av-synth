...
- `npm run qa:opSweep:thorough` — half-day (~6 h) sweep at 30 steps × 1500 ms × 3 source frames (t=0.4s, 1.0s, 1.6s), plus a 20 s temporal hold for every noisy op (curl/vortex/feedback/voidEater/modulate*/scroll/rotate/grain/selfMod). Asserts dead params, bounded variance during the temporal hold, and per-frame baseline drift against `qa/baselines/op-sweeps-thorough/`. Intended to run overnight.
