...
1. **Balanced interpolator (default): 8-point windowed sinc** (Kaiser-windowed, β = 8.6 ≈ -100 dB stopband). Precomputed table of polyphase coefficients indexed by fractional position quantised to 1/256 (i.e. 256 phases × 8 taps = 2048 floats, ~8 KB). Per-sample cost: 8 multiply-adds.
2. **Fallback interpolator: 4-point Hermite** for high-density grain clouds where CPU pressure forces a downgrade. The runtime still switches automatically by budget, but the shipped UI now exposes a coarse **quality selector**: `eco` lowers the interpolation budget, `balanced` preserves the original 8-tap behaviour, and `high` raises the fallback budget and prefers a 12-tap sinc path before Hermite. The shipped instrument also exposes runtime diagnostics (requested/effective quality, interpolation mode, budget pressure, voice load) and may optionally auto-step requested `high` down to the reduced-tap sinc path before conceding Hermite. This control is system-facing rather than modulation-facing.
