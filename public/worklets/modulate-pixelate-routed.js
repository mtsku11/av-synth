...
class RoutedModulatePixelateProcessor extends AudioWorkletProcessor {
  ...
  process(inputs, outputs, parameters) {
    ...
    for (let i = 0; i < output[0].length; i++) {
      ...
      const modSample = clamp((Math.abs(modLeft) + Math.abs(modRight)) * 0.5, 0, 1);
...
registerProcessor('modulate-pixelate-routed', RoutedModulatePixelateProcessor);
